      ...
      constructor(client) {
          ...
          this.exec = async (message, args, prefix) => {
                  ...
                  if (query.match(this.client.spotify.spotifyPattern)) {
                      ...
                      if (result.loadType === `PLAYLIST_LOADED`) {
                          ...
                          if (!dispatcher) {
                              ...
                              dispatcher = new AvonDispatcher(this.client, message.guild, message.channel, player);
                  ...
                  if (result.loadType === `PLAYLIST_LOADED`) {
                      ...
                      return message.reply({
                          embeds: [
                              this.client.utils
                                  .premiumEmbed(message.guildId)
                                  .setDescription(`${this.client.emoji.queue} Added **[${result.tracks.length}](${this.client.config.voteUrl})** from [${result.playlistInfo.name ?? "Playlist"}](${this.client.config.voteUrl})`),
                          ],
                      });
                  }
              }
              ...
              if (type === "direct") {
                  ...
                  let node = this.client.shoukaku.getNode();
                  let b1 = this.client.utils.button(`custom_id`, null, 2, `avon_default_search`, null, this.client.emoji.defSearch);
                  let b2 = this.client.utils.button(`custom_id`, null, 3, `avon_spoti_search`, null, this.client.emoji.spotiSearch);
                  let b3 = this.client.utils.button(`custom_id`, null, 4, `avon_deez_search`, null, this.client.emoji.deezSearch);
                  let b4 = this.client.utils.button(`custom_id`, null, 1, `avon_sound_search`, null, this.client.emoji.soundSearch);
                  let buttons = [b1, b2, b3, b4];
                  let row = this.client.utils.actionRow(buttons);
                  let em = this.client.utils
                      .premiumEmbed(message.guildId)
                      .setTitle(`Search Engines`)
                      .setDescription(`*Choose One of the buttons below to prefer your search type.*\n\n> ${this.client.emoji.defSearch} - Youtube\n> ${this.client.emoji.spotiSearch} - Spotify\n> ${this.client.emoji.deezSearch} - Deezer\n> ${this.client.emoji.soundSearch} - SoundCloud\n\n Note: Youtube engine is temprory.`);
